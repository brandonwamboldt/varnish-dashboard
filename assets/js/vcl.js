        ...
        app.get(app.getCurrentServer(), '/vcljson/', function(response) {
            ...
            for (var i = 0; i < response.vcls.length; i++) {
                ...
                if (current_vcl == response.vcls[i].name || (!current_vcl && response.vcls[i].status == 'active')) {
                    html += '<td><a data-vcl="' + response.vcls[i].name + '" class="view-vcl btn btn-xs btn-success disabled">View</a></td>';
                } else {
                    html += '<td><a data-vcl="' + response.vcls[i].name + '" class="view-vcl btn btn-xs btn-success">View</a></td>';
                }
